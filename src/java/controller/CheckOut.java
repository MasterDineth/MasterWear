...
public class CheckOut extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (user == null) {
            ...
            if (isCurrentAddress) {
                Criteria c1 = hibernateSession.createCriteria(UserData.class);
                c1.add(Restrictions.eq("id", user.getId()));
                c1.addOrder(Order.desc("id"));
                if (c1.list().isEmpty()) {
                    responseObject.addProperty("message",
                            "You current address is not found. Please add a new address");
                } else {
                    UserData address = (UserData) c1.list().get(0);
                    processCheckout(hibernateSession, hibernateTransaction, user, address, responseObject);
                }
            } else {
                if (firstName.isEmpty()) {
                    ...
                    if (city == null) {
                        ...
                        if (lineOne.isEmpty()) {
                            ...
                            userData.getAddress().setLine1(lineTwo);
                            userData.getAddress().setLine2(lineTwo);
                            userData.getAddress().setCity(city);
                            userData.getAddress().setPostalCode(postalCode);
                            userData.setMobile(mobile);
                            userData.setUserID(user.getId());
                            hibernateSession.save(userData);

                            processCheckout(hibernateSession, hibernateTransaction, user, userData, responseObject);
                            System.out.println("process sent to checkout");
                        }
                    }
                }
            }
...
//                orderItems.setRating(CheckOut.RATING_DEFAULT_VALUE); // 0
            ...
            //PayHere process
            String merahantID = "1226859";
            String merchantSecret = "MzgzMjY0ODYwMzE0ODAxOTgyMzg0NjM3Nzc0MzMyOTUyODg4NDg4";
            ...
            payHereJson.addProperty("return_url", "http://localhost:8080/MasterWear/thankyou.html");
            ...
            payHereJson.addProperty("first_name", userData.getFirstName());
            payHereJson.addProperty("last_name", userData.getLastName());
            payHereJson.addProperty("email", user.getEmail());

            payHereJson.addProperty("phone", userData.getMobile());
            payHereJson.addProperty("address", userData.getAddress().getLine1()+ ", " + userData.getAddress().getLine2());
            payHereJson.addProperty("city", userData.getAddress().getCity().getName());
