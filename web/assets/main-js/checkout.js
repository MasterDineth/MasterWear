...
async function loadCheckoutData() {
    ...
    const response = await fetch("/MasterWear/LoadCheckOutData");
    if (response.ok) { //200
...
//            const deliveryTypes = json.deliveryTypes;
            ...
            current_address_checkbox.addEventListener("change", function () {
                ...
                let postal_code = document.getElementById("zipCode");
                let mobile = document.getElementById("mobile");

                if (current_address_checkbox.checked) {
                    first_name.value = userData.firstName;
                    last_name.value = userData.lastName;
                    city_select.value = userData.address.city.id;
                    city_select.disabled = true;
                    city_select.dispatchEvent(new Event("change"));
                    line_one.value = userData.address.line1;
                    line_two.value = userData.address.line2;
                    postal_code.value = userData.address.postalCode;
...
//                }
            ...
            if (json.message === "empty-cart") {
                ...
                console.log("Empty cart. Please add some product");
        ...
        if (response.status === 401) {
            window.location = "html/auth-login-basic.html";
        }
    }
}
...
async function checkout() {
    ...
    const response = await fetch("/MasterWear/CheckOut", {
        method: "POST",
        header: {
